...
Single-file HTML PWA (~3,824 lines). Audiobook/ebook reader with synced word-level highlighting.
...
**Approximate line ranges (index.html):** CSS `16–511` · HTML `513–907` · JS `908–3822`
...
| Folder confirm sheet | `.folder-confirm-sheet`, `.folder-confirm-name`, `.folder-confirm-sub` (PWA folder change warning sheet) |
...
| `#optPanel` | Flyout options: 3 tabs (Playback, Display, Advanced) with sliders/toggles, rate presets, auto-scroll, highlight mode pills (`#hlPill-off`/`-sentence`/`-word`), resync, stop, sync offset (`#offsetRow`). About section includes `#aboutVersion`. Settings with ⓘ icons show inline `.op-desc` help text on tap via `toggleOpInfo()` |
...
| **DISPLAY PREFERENCES** | `saveDisplayPrefs()` (reads `_fontBody/_fontSize/_lineHeight/_maxWidth`; saves `sentHl`/`wordHl`/`hlMode`; no `getComputedStyle`), `loadDisplayPrefs()` (restores `sentHlOn`/`wordHlOn` from booleans for backward compat, then overrides with `hlMode` if present; calls `_syncHlPills()`; if saved `fontKey==='dyslexic'`, injects CDN `<link>` with `onload` repaint — same guard as `setFont()`) |
| **LIBRARY UI** | `renderLib()` (Add Book card gated by `!IS_PWA`; in PWA mode renders collapsible "Hidden books" section at bottom if `_hiddenBooks.length>0`), `renameBook()` (inline edit field on card — no `window.prompt()`; blur/Enter saves, Escape cancels; PWA mode persists title to `PWA_PROG_KEY`), `deleteBook()` (inline confirm; browser mode: calls `_deleteBlobsFor` + removes from library; PWA mode: writes `hidden:true` to `PWA_PROG_KEY`, moves book to `_hiddenBooks`), `unhideBook(id)` (removes `hidden` flag from `PWA_PROG_KEY`, moves book back to `library` in sorted order), `configurePlayerForMode()` |
...
| **PWA FILE SYSTEM** | `_resolveAmbiguousTranscripts()` (iterates `library` for books with `_unresolvedJsonCandidates`; awaits `_pickJsonFile()` for each), `_pickJsonFile(bookTitle,candidates)` (returns Promise; bottom sheet with per-file buttons + Skip; uses `xh()` for title/filename escaping), `pwaFolderChangeTap()` (pre-pick warning sheet shown when changing an existing folder — warns about Android back-button behaviour; Cancel dismisses, "Open folder picker" calls `pwaPickFolder()`; only wired on `#pwaFolderBadge` onclick, not on first-run or regrant screens), `pwaPickFolder()` (calls `showDirectoryPicker()`, commits handle immediately — no post-pick confirmation), `pwaRegrantAccess()`, `pwaScanAndRender()` (per-folder try/catch around `pwaScanBookFolder`; after merge step, splits books into `library` (visible) and `_hiddenBooks` (books with `hidden:true` in saved progress); calls `renderLib()` then `await _resolveAmbiguousTranscripts()`), `pwaScanBookFolder()` (skips files with base name 'metadata'; tiered JSON transcript auto-assignment: Tier 1 filename contains 'transcript'/'whisper', Tier 2 filename contains folder-name word 3+ chars, Tier 3 lone JSON fallback; unresolved multi-JSON flagged via `_unresolvedJsonCandidates`), `getPwaProgress()`, `savePwaProgress()` (prog includes `syncOffset`; uses `Object.assign` which preserves `hidden` field), `pwaOpenBook()` (now awaits `loadTranscriptData` before reading ebook handle; shows saved progress % immediately on open; shows toasts on audio/ebook handle failure; transcript catch nulls `transcriptData`/`transcriptType`) |
...
| `_hiddenBooks` | `Array<Object>` | Books with `hidden:true` in PWA saved progress; populated by `pwaScanAndRender()` after merge; rendered in collapsible section by `renderLib()` |
...
**Display prefs** saved separately to `folio_display_prefs_v1` (theme, fonts, font-size, line-height, width, alignment, wpm, sentPauseMs, wordHlOn, sentHlOn, hlMode). `hlMode` (`'off'`/`'sentence'`/`'word'`) is the canonical highlight state; `wordHl`/`sentHl` booleans are kept for backward compat with old saved prefs.
...
```
{
  ...
  hidden,                     // PWA-only: true if book is hidden; stored in PWA_PROG_KEY, merged by pwaScanAndRender
}
```
...
25. **PWA hide uses `Object.assign` preservation**: `savePwaProgress()` and `flushPositionSync()` use `Object.assign(existing, prog)` which preserves `hidden:true` because `prog` never sets a `hidden` field. Do not add `hidden` to the `prog` object in those functions or it will overwrite the hidden state. `unhideBook()` explicitly `delete`s `all[id].hidden` rather than setting it to `false`, keeping the saved progress object clean.
26. **`_resyncAndHL()` uses reverse linear scan on sparse `sentenceTimings`**: Consistent with `timeupdate` and `onSeekChange` handlers (see fragile area #13). Uses `isFinite(_audio.currentTime)` guard for edge cases where audio is not yet loaded. Sets `curWord=-1` sentinel after resync to prevent word-0 flash (see fragile area #14). Only resyncs in audio mode — TTS mode's `curSent` is always accurate since `ttsPlay` advances it directly.
27. **`_hiddenBooks` is repopulated on every `pwaScanAndRender()`**: Not persisted — derived from `PWA_PROG_KEY` on each scan. `deleteBook()` in PWA mode pushes to `_hiddenBooks` for immediate UI update without requiring a rescan. `unhideBook()` splices from `_hiddenBooks` and pushes back to `library` with re-sort.
